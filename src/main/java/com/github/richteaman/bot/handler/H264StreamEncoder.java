package com.github.richteaman.bot.handler;
...
import com.github.richteaman.bot.handler.frame.FrameEncoder;
...
public class H264StreamEncoder extends OneToOneEncoder{
	...
	protected Object encode(ChannelHandlerContext ctx, Channel channel,
		...
		return encode((BufferedImage)msg);
	}

	public ChannelBuffer encode(BufferedImage bufferedImage) throws Exception {
		...
		if (iPacket.isComplete()) {
			...
			try{
				...
				ChannelBuffer channelBuffer = ChannelBuffers.copiedBuffer(byteBuffer.order(ByteOrder.BIG_ENDIAN));
				if (frameEncoder != null) {
					return frameEncoder.encode(channelBuffer);
				}
