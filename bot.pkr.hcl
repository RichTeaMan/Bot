...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      git clone --branch ${local.git_branch} git@github.com:RichTeaMan/Bot.git'
      EOF
      ,
      "chown ${local.username}:${local.username} /home/${local.username}/ -R",
      "su -l ${local.username} -c './build.sh'"
    ]
  }
}
